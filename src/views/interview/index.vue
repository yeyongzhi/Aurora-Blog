...
const BASE_ARTICLE_PATH = '/article/interview'
...
const initTreeData = async () => {
    try {
        const data = await getFetchData('/interview.json')
        treeData.value = data
        ...
        console.error('获取面试笔记目录失败:', error)
    }
}
...
watch(
    ...
    (newPath) => {
        ...
        if (appStore.menuKey !== 'interview') return
