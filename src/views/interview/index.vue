...
const { articleMissing, treeData, noteKey, mdFilePath, treeError, treeLoading, initTreeData, handleArticleChanged, slideMenuVisible, toggleSlideMenu } = useArticleCollection('interview')
...
<template>
    <div class="w-full h-full p-2 sm:p-4 flex flex-col lg:flex-row gap-4">
        ...
        <div class="border flex flex-col min-w-0 min-h-0 flex-1 rounded-xl p-2 sm:p-4">
            ...
            <MarkDown v-else class="min-h-0 flex-1 !h-auto" :path="mdFilePath" :showInfo="false" />
        </div>
    </div>
</template>
