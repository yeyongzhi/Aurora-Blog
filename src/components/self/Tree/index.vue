...
<template>
    <div class="w-full h-full">
        <div class="h-full flex justify-center items-center" v-if="props.data.length === 0">
            <Empty>
                <EmptyHeader>
        ...
        <ScrollArea class="w-full h-full" v-else>
            <div class="rounded-md border px-4 py-2 text-sm mb-2 flex flex-col" v-for="item in props.data"
                :key="item.key">
                <Collapsible>
                    <CollapsibleTrigger>
                    ...
                    <CollapsibleContent>
                        <div class="mt-2 flex flex-col gap-y-2 pl-4" v-if="item.children && item.children.length > 0">
                            <div class="rounded-md border px-2 py-1 text-sm flex items-center justify-between"
