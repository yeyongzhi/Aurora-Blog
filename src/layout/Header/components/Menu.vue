            ...
            <NavigationMenuList>
                <NavigationMenuItem v-for="item in APP_MENU" :key="item.key">
                    <NavigationMenuLink class="cursor-pointer" as-child v-if="!item.children" @click="appStore.handleMenuChange(item.key)">
                        <button type="button" :class="`w-fit whitespace-nowrap ${appStore.menuKey === item.key ? 'font-extrabold underline' : ''}`">{{ item.name }}</button>
                    </NavigationMenuLink>
                    <template v-else>
                        ...
                        <NavigationMenuContent>
                            <NavigationMenuLink class="cursor-pointer" v-for="child in item.children" :key="child.key" @click="appStore.handleMenuChange(child.key)">
                                <button type="button" :class="`text-center w-fit whitespace-nowrap ${appStore.menuKey === child.key ? 'font-extrabold underline' : ''}`">{{ child.name }}</button>
                            </NavigationMenuLink>
                        </NavigationMenuContent>
                    </template>
                </NavigationMenuItem>
